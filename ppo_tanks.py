...
class ActorCritic(nn.Module):
    def __init__(self, num_actions, observation_shape):
        ...
        input_size = 0
        for space in observation_shape:
            if isinstance(space, gym.spaces.Box):
                input_size += np.prod(space.shape)
            elif isinstance(space, gym.spaces.Sequence):
                # Assuming the first element is of the same type and shape as the rest
                if hasattr(space, "feature_space"):
                    input_size += np.prod(space.feature_space.shape) * 2
                else:
                    # If not feature_space and it's a sequence, then it is an observation space
                    # Assuming each element in sequence has the same Box space
                    # Can take any element in the sequence, they should have same shape
                    
                    input_size += np.prod(space.sample().shape) * 2
...
def train_ppo():
	...
	model = ActorCritic(num_actions, observation_shape)
